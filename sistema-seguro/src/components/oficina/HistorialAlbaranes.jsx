  ...
  const convertirUrlABase64 = (url) => {
      return new Promise((resolve) => {
          ...
          img.onload = () => {
              ...
              ctx.fillStyle = `${color.superficie}`; ctx.fillRect(0, 0, canvas.width, canvas.height);
      ...
      <div style={blockStyle}>
          {partePreview && (
              <Modal
                  ...
                  onCerrar={() => setPartePreview(null)}
                  acciones={<>
                      <Boton variante="secundario" onClick={() => setPartePreview(null)}>Cancelar</Boton>
                      <Boton onClick={() => generarPDFAlbaran(partePreview)} disabled={generandoPDF}>
                          <Download size={16} /> {generandoPDF ? 'Generando...' : 'Descargar PDF'}
                      </Boton>
                  </>}
              >
                  <div style={{ fontSize: '13px', color: color.texto, display: 'flex', flexDirection: 'column', minHeight: '420px' }}>
                           <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px solid ${color.linea}`, paddingBottom: '15px', marginBottom: '15px' }}>
                               <div><p style={{ margin: '0 0 5px 0', fontSize: '10px', color: color.textoSuave, fontWeight: 'bold', textTransform: 'uppercase' }}>Proyecto</p><p style={{ margin: 0, fontSize: '16px', fontWeight: 'bold', textTransform: 'uppercase' }}>{partePreview.obra}</p></div>
                               <div style={{ textAlign: 'right' }}><p style={{ margin: '0 0 5px 0', fontSize: '10px', color: color.textoSuave, fontWeight: 'bold', textTransform: 'uppercase' }}>Fecha</p><p style={{ margin: 0, fontSize: '14px' }}>{partePreview.fecha}</p></div>
                           </div>

                           <div style={{ marginBottom: '20px' }}>
                               <p style={{ margin: '0 0 5px 0', fontSize: '10px', color: color.textoSuave, fontWeight: 'bold', textTransform: 'uppercase' }}>Personal Asignado</p>
                               <p style={{ margin: 0, padding: '10px', backgroundColor: color.fondo, border: `1px solid ${color.petroleo}` }}>{partePreview.cuadrilla?.length > 0 ? partePreview.cuadrilla.map(c=>c.nombre).join(' - ') : partePreview.nombreTrabajador}</p>
                           </div>

                           {partePreview.materialesUsados && partePreview.materialesUsados.length > 0 && (
                               <div style={{ marginBottom: '20px' }}>
                                   <p style={{ margin: '0 0 5px 0', fontSize: '10px', color: color.textoSuave, fontWeight: 'bold', textTransform: 'uppercase' }}>Material Empleado y Costes</p>
                                   <ul style={{ margin: 0, padding: '12px', backgroundColor: color.fondo, border: `1px solid ${color.petroleo}`, listStyle: 'none' }}>
                                       {partePreview.materialesUsados.map((m, i) => (
                                           <li key={i} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px dashed ${color.linea}`, paddingBottom: '5px', marginBottom: '5px' }}>
                           ...
                           <div style={{ marginBottom: '20px' }}>
                               <p style={{ margin: '0 0 5px 0', fontSize: '10px', color: color.textoSuave, fontWeight: 'bold', textTransform: 'uppercase' }}>Tareas y Habitaciones</p>
                               {partePreview.tareasRealizadas && partePreview.tareasRealizadas.length > 0 ? (
                                   <div style={{ backgroundColor: color.fondo, border: `1px solid ${color.petroleo}` }}>
                                       {partePreview.tareasRealizadas.map((t, i) => (
                                           <div key={i} style={{ padding: '10px', borderBottom: i !== partePreview.tareasRealizadas.length -1 ? `1px solid ${color.linea}` : 'none' }}>
                                               <strong style={{ display: 'block', color: color.texto }}>{t.ubicacion}</strong>
                                               <span style={{ color: color.textoSuave }}>{t.descripcion}</span>
                                           </div>
                                       ))}
                                   </div>
                               ) : (
                                   <p style={{ margin: 0, padding: '10px', backgroundColor: color.fondo, border: `1px solid ${color.petroleo}`, whiteSpace: 'pre-wrap' }}>{partePreview.trabajo || 'Sin observaciones'}</p>
                               )}
                           </div>

                           <div style={{ marginTop: 'auto', paddingTop: '20px' }}>
                               <p style={{ margin: '0 0 10px 0', fontSize: '10px', color: color.textoSuave, fontWeight: 'bold', textTransform: 'uppercase' }}>Firma de Conformidad</p>
                               <div style={{ width: '100%', height: '120px', backgroundColor: color.fondo, border: `1px dashed ${color.petroleo}`, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                   {partePreview.firma ? ((firmaPreview.id === partePreview.id && firmaPreview.dataUrl) ? <img src={firmaPreview.dataUrl} alt="Firma" style={{ maxHeight: '100px' }} /> : <span style={{ color: color.textoTenue, fontSize: '12px' }}>Cargando firma…</span>) : <span style={{ color: color.textoTenue, fontSize: '12px', textTransform: 'uppercase' }}>Sin firma registrada</span>}
                               </div>
                           </div>
                      </div>
              </Modal>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
              ...
              <button onClick={exportarPartesExcel} style={{ ...btnBlackStyle, backgroundColor: color.superficie, color: color.texto, border: `1px solid ${color.petroleo}` }}><FileSpreadsheet size={14} /> Exportar BBDD</button>
          </div>
          ...
          <div onScroll={(e) => { if (filtroBuscador !== '' || fechaInicio !== '' || fechaFin !== '') return; const { scrollTop, clientHeight, scrollHeight } = e.currentTarget; if (scrollHeight - scrollTop <= clientHeight + 30) setLimitePartes(prev => prev + 10); }} style={{ maxHeight: '450px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '15px', paddingRight: '5px' }}>
              {partesAMostrar.length === 0 ? <div style={{ fontSize: '12px', color: color.textoSuave, padding: '20px', textAlign: 'center', backgroundColor: color.fondo, border: `1px dashed ${color.canto}` }}>No se han encontrado albaranes.</div> : (
                  <>
                      {partesAMostrar.map((parte) => ( 
                        <div key={parte.id} style={{ padding: '25px', border: `1px solid ${color.petroleo}`, backgroundColor: color.superficie }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '15px', marginBottom: '15px' }}>
                                <h4 style={{ margin: '0', fontSize: '14px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>{parte.obra} <span style={{ fontSize: '11px', color: color.textoSuave, fontWeight: 'normal' }}>| {parte.fecha}</span></h4>
                                <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', alignItems: 'center' }}>
                                    <button onClick={() => setPartePreview(parte)} style={{ background: 'transparent', border: `1px solid ${color.petroleo}`, color: color.texto, padding: '4px 8px', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}><Eye size={12}/> Ver Albarán</button>
                                    {parte.facturado && <Insignia tono="fuerte">Facturado</Insignia>}
                                    {parte.certificado && !parte.facturado && <Insignia tono="info">Certificado</Insignia>}
                                    {!parte.facturado && !parte.certificado && <Insignia tono="neutra">Validado libre</Insignia>}
                                </div>
                            </div>
                            ...
                            <div style={{ fontSize: '12px', color: color.textoSuave, borderLeft: `3px solid ${color.petroleo}`, paddingLeft: '10px', marginTop: '10px' }}><em>{parte.tareasRealizadas?.length > 0 ? parte.tareasRealizadas.map(t => `${t.ubicacion}: ${t.descripcion}`).join(' | ') : (parte.trabajo || 'Sin observaciones')}</em></div>
                        </div>
                      ))}
                      ...
                      {hayMasPartes && (
                          <button 
                              ...
                              style={{ width: '100%', padding: '15px', backgroundColor: color.fondo, textAlign: 'center', fontSize: '11px', fontWeight: 'bold', color: color.texto, cursor: 'pointer', border: `1px dashed ${color.canto}`, textTransform: 'uppercase' }}
                          >
