...
export default function ControlNominas({ blockStyle, btnBlackStyle, labelStyle, inputStyle, fechaInicio, setFechaInicio, fechaFin, setFechaFin, pagoHoraNormal, setPagoHoraNormal, pagoHoraExtra, setPagoHoraExtra, horasTrabajadores }) {
  ...
  const datosCalculados = horasTrabajadores.map(([nombre, data]) => {
      ...
      return { nombre, hNormal, hExtra, tarifaN, tarifaE, totalPagar };
  });

  const totalGeneralNomina = datosCalculados.reduce((acc, item) => acc + item.totalPagar, 0);

  // EXCEL INTELIGENTE CON FILA DE TOTAL GLOBAL AL FINAL
  const exportarExcelPersonalizado = () => {
      ...
      // Añadimos la fila final con el total acumulado
  ...
  return (
      <div style={blockStyle}>
          ...
          {/* BARRA DE FILTROS Y TARIFAS GENERALES */}
          <div style={{ marginBottom: '25px', display: 'flex', gap: '15px', padding: '20px', border: '1px solid #e5e7eb', backgroundColor: '#fafafa', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              ...
              <div style={{ flex: 1, minWidth: '120px' }}><label style={labelStyle}>Tarifa Base Normal (€)</label><input type="number" step="0.5" value={pagoHoraNormal} onChange={(e) => setPagoHoraNormal(Number(e.target.value))} style={inputStyle} /></div>
              <div style={{ flex: 1, minWidth: '120px' }}><label style={{...labelStyle, color: '#2563eb'}}>Tarifa Base Extra (€)</label><input type="number" step="0.5" value={pagoHoraExtra} onChange={(e) => setPagoHoraExtra(Number(e.target.value))} style={{...inputStyle, borderColor: '#2563eb', color: '#2563eb', fontWeight: 'bold'}} /></div>
          </div>
                          ...
                          <div key={index} style={{ padding: '20px', border: '1px solid #1a1a1a', backgroundColor: '#ffffff', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                              ...
                              <div style={{ display: 'flex', gap: '10px', backgroundColor: '#fafafa', padding: '10px', border: '1px solid #e5e7eb', borderRadius: '4px' }}>
                                  <div style={{ flex: 1 }}>
                                      ...
                                      <input type="number" step="0.5" value={item.hNormal} onChange={(e) => handleHorasChange(item.nombre, 'horas', e.target.value, horasTrabajadores[index][1].horas, horasTrabajadores[index][1].horasExtra)} style={{ ...inputStyle, padding: '6px', fontSize: '11px', backgroundColor: '#fff' }} />
                                  </div>
                                  <div style={{ flex: 1 }}>
                                      <label style={{ fontSize: '9px', fontWeight: 'bold', color: '#2563eb', display: 'block', marginBottom: '3px' }}>H. EXTRAS</label>
                                      <input type="number" step="0.5" value={item.hExtra} onChange={(e) => handleHorasChange(item.nombre, 'horasExtra', e.target.value, horasTrabajadores[index][1].horas, horasTrabajadores[index][1].horasExtra)} style={{ ...inputStyle, padding: '6px', fontSize: '11px', backgroundColor: '#fff', borderColor: '#2563eb', color: '#2563eb', fontWeight: 'bold' }} />
                                  </div>
                              </div>
                              ...
                              <div style={{ display: 'flex', gap: '10px', backgroundColor: '#f8fafc', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '4px' }}>
                                  <div style={{ flex: 1 }}>
                                      ...
                                      <input type="number" step="0.5" value={item.tarifaN} onChange={(e) => handleTarifaChange(item.nombre, 'normal', e.target.value)} style={{ ...inputStyle, padding: '6px', fontSize: '11px', backgroundColor: '#fff' }} />
                                  </div>
                                  <div style={{ flex: 1 }}>
                                      <label style={{ fontSize: '9px', fontWeight: 'bold', color: '#2563eb', display: 'block', marginBottom: '3px' }}>€/H EXTRA</label>
                                      <input type="number" step="0.5" value={item.tarifaE} onChange={(e) => handleTarifaChange(item.nombre, 'extra', e.target.value)} style={{ ...inputStyle, padding: '6px', fontSize: '11px', backgroundColor: '#fff', borderColor: '#2563eb', color: '#2563eb', fontWeight: 'bold' }} />
                                  </div>
                              </div>
